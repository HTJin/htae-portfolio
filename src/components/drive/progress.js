...
function parseTip(raw, { allowSoftOutcomes }) {
  ...
  if (!outcomesPlain && !allowSoftOutcomes) {
    // Caller decides; for public read we soft-recover (R0p).
  }
  return {
    kind: 'ok',
    index,
...
function readV1Tip(store) {
  try {
    ...
    if (!raw) return null
    const tip = parseTip(raw, { allowSoftOutcomes: true })
...
export function readProgress() {
  ...
  try {
    ...
    if (v2Raw != null) {
      const tip = parseTip(v2Raw, { allowSoftOutcomes: true })
      if (tip.kind !== 'ok') return null
...
export function writeProgress(index, outcomesPatch) {
  ...
  try {
    ...
    if (v2Before != null) {
      const probe = parseTip(v2Before, { allowSoftOutcomes: true })
      if (probe.kind !== 'ok') {
        try {
    ...
    // After R0k heal, v2 is absent: still materialize tip from v1 (R0k / R0t).
